...
namespace BrightIdeasSoftware
{
  public class ToolTipControl : NativeWindow
  {
    ...
    internal int WindowStyle
    {
      get
      {
        return NativeMethods.GetWindowLong(this.Handle, -16);
      }
      set
      {
        NativeMethods.SetWindowLong(this.Handle, -16, value);
      }
    }
    ...
    public Color BackColor
    {
      get
      {
        return ColorTranslator.FromWin32((int) NativeMethods.SendMessage(this.Handle, 1046, 0, 0));
      }
      set
      {
        if (ObjectListView.IsVistaOrLater)
    ...
    public Color ForeColor
    {
      get
      {
        return ColorTranslator.FromWin32((int) NativeMethods.SendMessage(this.Handle, 1047, 0, 0));
      }
      set
      {
        if (ObjectListView.IsVistaOrLater)
    ...
    {
      return (int) NativeMethods.SendMessage(this.Handle, 1045, which, 0);
    }

    private void SetDelayTime(int which, int value)
    {
      NativeMethods.SendMessage(this.Handle, 1027, which, value);
    }
    ...
    {
      NativeMethods.SendMessageTOOLINFO(this.Handle, 1074, 0, this.MakeToolInfoStruct(window));
    }

    public void PopToolTip(IWin32Window window)
    {
      NativeMethods.SendMessage(this.Handle, 1052, 0, 0);
    }

    public void RemoveToolTip(IWin32Window window)
    {
      NativeMethods.SendMessageTOOLINFO(this.Handle, 1075, 0, this.MakeToolInfoStruct(window));
    }
    ...
    {
      NativeMethods.SendMessage(this.Handle, 1048, 0, maxWidth);
    }
    ...
    public virtual bool HandleGetDispInfo(ref Message msg)
    {
      ...
      NativeMethods.NMTTDISPINFO nmttdispinfo = (NativeMethods.NMTTDISPINFO) msg.GetLParam(typeof (NativeMethods.NMTTDISPINFO));
      nmttdispinfo.lpszText = showingEventArgs.Text;
      nmttdispinfo.hinst = IntPtr.Zero;
      if (showingEventArgs.RightToLeft == RightToLeft.Yes)
        nmttdispinfo.uFlags |= 4;
      Marshal.StructureToPtr((object) nmttdispinfo, msg.LParam, false);
      return true;
    }
