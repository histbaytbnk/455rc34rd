...
namespace ICSharpCode.SharpZipLib.Zip
{
  public class ZipOutputStream : DeflaterOutputStream
  {
    ...
    {
      byte[] numArray = ZipConstants.ConvertToArray(comment);
      if (numArray.Length > (int) ushort.MaxValue)
        throw new ArgumentOutOfRangeException("comment");
      this.zipComment = numArray;
    }
    ...
    public void PutNextEntry(ZipEntry entry)
    {
      ...
      this.offset += (long) (30 + buffer.Length + entryData.Length);
      if (entry.AESKeySize > 0)
    ...
    public void CloseEntry()
    {
      ...
      if (this.curEntry.Size < 0L)
        ...
        throw new ZipException(string.Concat(new object[4]
        {
          (object) "size was ",
          (object) this.size,
          (object) ", but I expected ",
          (object) this.curEntry.Size
        }));
      if (this.curEntry.CompressedSize < 0L)
        ...
        throw new ZipException(string.Concat(new object[4]
        {
          (object) "compressed size was ",
          (object) num,
          (object) ", but I expected ",
          (object) this.curEntry.CompressedSize
        }));
      if (this.curEntry.Crc < 0L)
        ...
        {
          (object) "crc was ",
          (object) this.crc.Value,
          (object) ", but I expected ",
          (object) this.curEntry.Crc
        }));
      this.offset += num;
    ...
    public override void Finish()
    {
      ...
      foreach (ZipEntry entry in this.entries)
      {
        ...
        if (entry.AESKeySize > 0)
          ZipOutputStream.AddExtraDataAES(entry, extraData);
        byte[] entryData = extraData.GetEntryData();
        byte[] buffer2 = entry.Comment != null ? ZipConstants.ConvertToArray(entry.Flags, entry.Comment) : new byte[0];
        ...
        if (buffer1.Length > 0)
          this.baseOutputStream_.Write(buffer1, 0, buffer1.Length);
        if (entryData.Length > 0)
          this.baseOutputStream_.Write(entryData, 0, entryData.Length);
        if (buffer2.Length > 0)
          this.baseOutputStream_.Write(buffer2, 0, buffer2.Length);
        sizeEntries += (long) (46 + buffer1.Length + entryData.Length + buffer2.Length);
      }
      using (ZipHelperStream zipHelperStream = new ZipHelperStream(this.baseOutputStream_))
        zipHelperStream.WriteEndOfCentralDirectory(noOfEntries, sizeEntries, this.offset, this.zipComment);
