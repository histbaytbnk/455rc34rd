...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
  public class DeflaterHuffman
  {
    ...
    public void FlushBlock(byte[] stored, int storedOffset, int storedLength, bool lastBlock)
    {
      ...
      {
        if ((int) this.blTree.length[DeflaterHuffman.BL_ORDER[index]] > 0)
          blTreeCodes = index + 1;
      }
      int num1 = 14 + blTreeCodes * 3 + this.blTree.GetEncodedLength() + this.literalTree.GetEncodedLength() + this.distTree.GetEncodedLength() + this.extra_bits;
      int num2 = this.extra_bits;
      for (int index = 0; index < 286; ++index)
        num2 += (int) this.literalTree.freqs[index] * (int) DeflaterHuffman.staticLLength[index];
      for (int index = 0; index < 30; ++index)
        num2 += (int) this.distTree.freqs[index] * (int) DeflaterHuffman.staticDLength[index];
      if (num1 >= num2)
        num1 = num2;
    ...
    private class Tree
    {
      ...
      public void BuildTree()
      {
        ...
        {
          int num3 = (int) this.freqs[index1];
          if (num3 != 0)
          {
            int index2;
            int index3;
            for (index2 = num1++; index2 > 0 && (int) this.freqs[numArray1[index3 = (index2 - 1) / 2]] > num3; index2 = index3)
        ...
        do
        {
          ...
          int num3 = numArray2[index2];
          int index5;
          while ((index5 = index3) > 0 && numArray2[numArray1[index3 = (index5 - 1) / 2]] > num3)
      ...
      private void BuildLength(int[] childs)
      {
        ...
        int num4 = 2 * num1;
        for (int index2 = this.maxLength; index2 != 0; --index2)
        {
          int num3 = this.bl_counts[index2 - 1];
          while (num3 > 0)
          {
            int index3 = 2 * childs[num4++];
            if (childs[index3 + 1] == -1)
            {
              this.length[childs[index3]] = (byte) index2;
              --num3;
            }
          }
        }
