  ...
  internal class ZlibBaseStream : Stream
  {
    ...
    private void finish()
    {
      ...
      else
      {
        ...
        if (crc32Result != num1)
          throw new ZlibException(string.Format("Bad CRC32 in GZIP trailer. (actual({0:X8})!=expected({1:X8}))", (object) crc32Result, (object) num1));
        if (num3 != num2)
          throw new ZlibException(string.Format("Bad size in GZIP trailer. (actual({0})!=expected({1}))", (object) num3, (object) num2));
      }
    }
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      if (this._z.AvailableBytesOut > 0)
      {
        if (num1 == 0)
        {
          int num2 = this._z.AvailableBytesIn;
        }
        if (this.nomoreinput && this._wantCompress)
        {
          int num3 = this._z.Deflate(FlushType.Finish);
          switch (num3)
          {
            case 0:
            case 1:
              break;
            default:
              throw new ZlibException(string.Format("Deflating:  rc={0}  msg={1}", (object) num3, (object) this._z.Message));
          }
        }
      }
